...
export async function updateSite(
  ...
  await connectToDatabase()
  return Site.findByIdAndUpdate(id, { ...updateData, updatedAt: new Date() }, { new: true }).exec()
}
...
export async function incrementSiteViews(siteId: string) {
  await connectToDatabase()
  return Site.findByIdAndUpdate(siteId, { $inc: { views: 1 } }, { new: true }).exec()
}

export async function getAnalyticsBySiteId(siteId: string, startDate?: Date, endDate?: Date) {
  await connectToDatabase()
  const query: { siteId: string; date?: { $gte: Date; $lte: Date } } = { siteId }

  if (startDate && endDate) {
    query.date = { $gte: startDate, $lte: endDate }
  }

  return Analytics.find(query).sort({ date: -1 }).exec()
}

export async function getAnalyticsByUserId(userId: string, startDate?: Date, endDate?: Date) {
  await connectToDatabase()
...
export async function createAnalyticsEntry(analyticsData: {
  siteId: string
  userId: string
  date: Date
  views?: number
...
export async function getSiteVisits(siteId: string, startDate?: Date, endDate?: Date) {
  await connectToDatabase()

  try {
    const query: { siteId: mongoose.Types.ObjectId; visitedAt?: { $gte: Date; $lte: Date } } = { siteId: new mongoose.Types.ObjectId(siteId) }
...
export async function getUniqueVisitorsCount(siteId: string, startDate?: Date, endDate?: Date) {
  await connectToDatabase()
  const query: { siteId: mongoose.Types.ObjectId; visitedAt?: { $gte: Date; $lte: Date } } = { siteId: new mongoose.Types.ObjectId(siteId) }
